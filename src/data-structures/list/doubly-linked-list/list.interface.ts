export interface DoublyLinkedList<T> {
    ...
    /**
     * Removes the first element from an list and returns that element.
     */
    shift(): Promise<T | undefined>;

    /**
     * Adds one element to the beginning of an list and returns the new length of the list.
     */
    unshift(item: T): Promise<number>;
}
