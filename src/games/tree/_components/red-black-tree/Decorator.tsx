...
function loadTexts<T>(root: PositionNode<T>, renderer: THREE.Renderer, camera: THREE.Camera, scene: THREE.Scene) {
    ...
    function helper<T>(node: PositionNode<T>, font: Font) {

        const content = node.val + "";
        const geometry = new TextGeometry(content, { font: font, size: textFont.size, height: textFont.height, });
...
const Tree = <T extends unknown>({ root, marginY, scene, renderer, camera, startY, z, radius, plane, light }: Props<T>) => {
