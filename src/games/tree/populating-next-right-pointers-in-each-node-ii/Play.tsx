...
const getNext = (map: Map<TreeNode<string>, TreeNode<string>>, node?: TreeNode<string>): TreeNode<string> | undefined => {
    if (!node) {
        return undefined;
    }
...
const Main = () => {
    ...
    const doClick = (step: Step) => {
        ...
        const next = getNext(nextMap, nextMap.get(node));
        ...
        if (left) {
            if (right) {
                ...
                if (next.left) {
                    connect(left, next.left, scene, setNextMap);
                } else if (next.right) {
                    connect(left, next.right, scene, setNextMap);
                }
            }
