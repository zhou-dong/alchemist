...
const getTableSize = (items: RodCuttingItem[], totalLength: number): TableSize => {
    const rows = items.length + 1;
    const cols = totalLength + 1;
    return { rows, cols };
};

const createDPTable = (items: RodCuttingItem[], totalLength: number): number[][] => {
    const { rows, cols } = getTableSize(items, totalLength);
