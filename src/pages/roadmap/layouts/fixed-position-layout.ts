...
export const getFixedTreeLayout = (canvasWidth: number, canvasHeight: number): CategoryCircle[] => {
    ...
    return [
        ...
        buildCategoryCircle(CategoryType.LinkedList, centerX - 1.5 * radius, marginTop + 7 * radius),
        buildCategoryCircle(CategoryType.BinarySearch, centerX + 3 * radius, marginTop + 4 * radius),
        buildCategoryCircle(CategoryType.Recursion, centerX + 7.5 * radius, marginTop + radius + 9 * radius),
        buildCategoryCircle(CategoryType.DP, centerX + 6 * radius, marginTop + radius + 12 * radius),
        buildCategoryCircle(CategoryType.Heap, centerX - 1.5 * radius, marginTop + 10 * radius),
        buildCategoryCircle(CategoryType.SegmentTree, centerX + 4.5 * radius, marginTop + 10 * radius),
        buildCategoryCircle(CategoryType.UnionFind, centerX + 1.5 * radius, marginTop + 10 * radius),
        buildCategoryCircle(CategoryType.Graph, centerX - 4.5 * radius, marginTop + radius + 9 * radius),
        buildCategoryCircle(CategoryType.TopologicalSort, centerX - 3 * radius, marginTop + radius + 12 * radius),
    ];
}
